...
# =============================================================================
    ...
    if st.checkbox('Zijn er mensen die per se een bepaalde taak moeten afronden?'):
        ...
        for i in range(int(aantal_taken)):
            ...
            speciale_mensen = st.multiselect(
                 "".join(["wie gaan deze ",str(i+1),"e taak uitvoeren?"]),new_df['Werknemers'][(new_df["Aanwezig"]==1)& (new_df[speciale_taak]!=4)])
    ...
    if st.checkbox('Staat laden/lossen voor Zeelandia vandaag op de planning?',value=True):
        zeelandia = st.multiselect(
            'Wie gaat laden/lossen?',new_df['Werknemers'][new_df["Aanwezig"]==1])
...
# =============================================================================
            ...
            if (status==OptimizationStatus.OPTIMAL) or (status_relax==OptimizationStatus.OPTIMAL):
                ...
                st.dataframe(pd.concat([df_links,df_rechts]))
...
# =============================================================================
            ...
            def to_excel(df):
                ...
                row_under_last = str(endrow_r)
                format_top_border = workbook.add_format({'top':1})
                bereik = "".join(['A',row_under_last,':',num_to_let(endcol+2),row_under_last])
                worksheet.conditional_format(bereik, {'type':'cell',
                                    'criteria': 'not equal to','value': 50,'format': format_top_border})
                row_under_last = str(len(afwezig)+startrow_l)
                worksheet.conditional_format("".join([num_to_let(endcol+3),row_under_last]), {'type':'cell',
                                    'criteria': 'not equal to','value': 50,'format': format_top_border})
                ...
                writer.sheets['Planning'].set_column(0, 0, maximum_index + 1,bold_format)
                ...
                #url = 'https://raw.githubusercontent.com/emile-stage/Stage/main/NOVO-Logo.png'#'https://raw.githubusercontent.com/NovoPW/Planningstool/NOVO-Logo.png'
                #url = 'https://raw.githubusercontent.com/NovoPW/Planningstool/NOVO-Logo.png'
                ...
                worksheet.conditional_format('A1:Z30', {'type':'cell',
